    ...
    if (request.method === "POST") {
      try {
        ...
        if (fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
          ...
          if (!visionRes.ok) throw new Error(`Vision error: ${JSON.stringify(visionData)}`);
...
Suggest next steps if something looks wrong.`;
        ...
        if (isPaid) {
          paidFeatures = {
            downloadablePdf: true, // Frontend can generate PDF
            redFlags: extractRedFlags(fullExplanation),
            codeExplanations: extractCodes(fullExplanation),
            costComparison: mockCostComparison(), // Replace with real API later
            appealLetter: generateAppealLetter(fullExplanation),
            savedHistory: true, // KV save in frontend
